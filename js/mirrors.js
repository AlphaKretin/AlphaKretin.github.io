...
function move(direction, startPoint) { //direction: 0=up, 1= right, 2=down, 3=left //startPoint = {row: "A" column: 1}
    ...
    function colourAssign() {
        ...
        switch (direction) {
            case 0:
                destination.row = decrementLetter(currentCell.row);
                destination.column = currentCell.column;
                break;
            case 1:
                destination.column = currentCell.column + 1;
                destination.row = currentCell.row;
                break;
            case 2:
                destination.row = incrementLetter(currentCell.row);
                destination.column = currentCell.column;
                break;
            case 3:
                destination.column = currentCell.column - 1;
...
document.getElementById("B2").addEventListener("click", function() { //to be called onclick
    ...
    switch (B2state) {
        ...
        default:
            B2image = "blank";
            alert("state out of bounds");
            break; //should never happen, hence why i have it tell me if so
    }
